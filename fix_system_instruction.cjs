...
code = code.replace(/3\. NO GENERIC TERMS: Do not say "\$settings\.lighting". Say "\$\{settings\.lighting\}"\./, '3. NO GENERIC TERMS: Do not say "Good lighting". Say "${settings.lighting}".' + injection);

fs.writeFileSync('services/geminiService.ts', code);
console.log("Updated system instruction for single prompt");
