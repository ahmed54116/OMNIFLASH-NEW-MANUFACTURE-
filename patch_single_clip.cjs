...
const singleClipRegex = /const systemInstruction = \`You are the Prompt Director[\s\S]*?\$\{continuityContext\}\`;/;
const oldMatch = code.match(singleClipRegex);
if (oldMatch) {
    const oldCode = oldMatch[0];
    const newCode = `const systemInstruction = (typeof settings !== 'undefined' && settings.manufacturingJson && settings.manufacturingJson.trim().length > 0) ? \`You are an elite cinematic director and AI video prompt engineer.
...
2. SCRIPT SYNC: Use the provided script line exactly. DO NOT INVENT NEXT ACTIONS.\` : \`You are the Prompt Director` + oldCode.substring(53);

    code = code.replace(singleClipRegex, newCode);
    fs.writeFileSync('services/geminiService.ts', code);
    console.log("Patched generateSingleClip");
} else {
    console.log("Regex not found for singleClipRegex");
}
