...
code = code.replace(/const detailedJsonSchema: any = \{[\s\S]*?if \(settings\.generateImageAndAnimationPrompts\) \{/m, `const detailedJsonSchema: any = {
    type: Type.OBJECT,
    properties: {
...
code = code.replace(/const clipSchema = \{[\s\S]*?const batchSchema = \{/m, `const clipSchema = {
    type: Type.OBJECT,
    properties: {
...
console.log("Updated JSON schemas successfully");
